...
if(global.game){
	...
	{
	    show_debug_message(
	        "obj_camera does not exist, creating it"
	    );

	    if (instance_exists(obj_player))
	    {
	        show_debug_message(
	            "obj_player exists, assigning it as camera target"
	        );

	        var app = instance_create_depth(
	            0,
	            0,
	            depth,
	            obj_camera
	        ...
	        app.target = obj_player;
	    }
	    else
	    {
	        show_error(
	            "Obj Player was not created in the correct order",
	            true
	        );
	    }
	}
	...
	if(global.game_ini_amount == 0 and alarm[0]==-1 and room!=room_debug){
		go_to_next_stage()
	}
