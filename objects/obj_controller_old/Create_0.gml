...
//Game Control (Game Level stuff is reset at every stage of the game)
...
//Reloads the entire session (a session is a run of the game) 
function reload_session(){
	show_debug_message("Reloading Session->Restarting game")
	global.game_stage = 1;
	scr_reload() //reload numeric data
	reload_game()
}

//Reloads after every stage
function reload_game(){
	show_debug_message("Reloading Game")
	...
	//Game's only true after everything before
	global.game = true;
